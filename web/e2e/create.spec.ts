...
test('create an inGitDB database from Home, see it listed, then remove it and keep its data', async ({ browser }) => {
  ...
  await expect(steps.getByText('Connect with a manifest file')).toBeVisible()
...
test('creating over a folder with files is refused with next steps, and nothing is written (AC:create-refuses-overwrite)', async ({ browser }) => {
  ...
  await expect(alert.getByText('Use the name notes-web-2 instead')).toBeVisible()
  await expect(alert.getByText('ovdb databases create notes-web-2', { exact: true })).toBeVisible()
...
test('a SQLite result puts describing the schema first', async ({ browser }) => {
  ...
  await expect(first).toContainText('Describe your data first')
  await expect(first).toContainText('ovdb server restart')
